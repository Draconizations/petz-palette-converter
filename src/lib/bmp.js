...
function bmpDecoder(arrayBuffer) {
  const decoder = new BmpDecoder(arrayBuffer);
  const bmpData = decoder.getData();
  const width = new DataView(arrayBuffer).getInt32(18, true);
  const height = Math.abs(new DataView(arrayBuffer).getInt32(22, true));
  return new ImageData(new Uint8ClampedArray(bmpData.buffer), width, height, )
}
export default bmpDecoder;

export function bmpPalette(arrayBuffer) {
  const decoder = new BmpDecoder(arrayBuffer);
